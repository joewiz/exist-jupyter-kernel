...
async function main() {
  ...
  const handlers = {
    ...
    async execute(content, executionCount, jupyterSession) {
      ...
      // Extract cell name from metadata if present
      const cellName = content.metadata?.exist?.name;

      // Parse xqdoc @output directives and merge serialization options
      const { serialization: directive } = parseDirectives(code);
