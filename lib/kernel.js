...
import { parseDirectives, mergeSerialization, wrapDataCell } from "./directives.js";
...
async function main() {
  ...
  const handlers = {
    ...
    async execute(content, executionCount, jupyterSession) {
      ...
      // Parse xqdoc directives (@name, @data, @silent, @output)
      const { name: directiveName, dataFormat, silent, serialization: directive } = parseDirectives(code);
      ...
      const evalCode = dataFormat ? wrapDataCell(code, dataFormat) : code;
      ...
      const result = await client.execute(evalCode, jupyterSession, cellName, serialization);
      await log(`execute_result #${executionCount}: ${JSON.stringify(result).slice(0, 200)}`);

      // @silent: suppress output (still execute for caching side effects)
